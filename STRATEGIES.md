...
> 最後整理:2026-07-16。這份是**名詞對照參考**,不是實作規格;參數以程式碼為準(`backend/internal/cache/`)。
...
| `conv` | **冥王星** | `conv` | 4H | EMA200 同側橫盤 4 根、VRVP 找止盈、盈虧比 ≥1.5 | tpMomentum | RR ≥1.5 | `convergence.go` |

> ⚠️ 上表「公開策略」= 一般使用者在導覽列看得到的分頁,但 **星軌/超新星/銀河/冥王星 這四本的分頁其實掛在 VIP 群組**(`can('vip')`),推播則發給全體(導流用)。`main`/`gamble`/`emaonly`/`conv` 的 API 閘門分別是 V。
...
1. **「銀河」一詞兼兩義** — `emaonly` 策略顯示名是「銀河」;**但** 冥王星/均值回歸三本 共用的選幣池(`emaCoins`)註解裡也叫「銀河 coins」。看文件時請自行區分:**大寫「銀河策略」= emaonly;「銀河幣池」= 共用選幣池**。日後若清理,建議把幣池改稱「母池」。
...
4. **已移除的策略**(DB 舊列成孤兒,不載入、無害):`gamblehedge`(超新星保本,2026-07)、`pool`(30幣掃描池,2026-07-16)。
